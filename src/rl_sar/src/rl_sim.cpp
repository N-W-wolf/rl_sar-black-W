...
bool TrySetKeyboardFromDebugInput(Control &control, const std::string &input)
{
    if (input.size() == 1)
    {
        ...
        if (c >= '0' && c <= '9')
        {
            control.SetKeyboard(static_cast<Input::Keyboard>(static_cast<int>(Input::Keyboard::Num0) + (c - '0')));
            return true;
        }
        if (c >= 'a' && c <= 'z')
        {
            control.SetKeyboard(static_cast<Input::Keyboard>(static_cast<int>(Input::Keyboard::A) + (c - 'a')));
            return true;
        }
        if (c == ' ')
        {
            control.SetKeyboard(Input::Keyboard::Space);
            return true;
        }
    }
    ...
    {
        control.SetKeyboard(Input::Keyboard::Enter);
        return true;
    }
    if (input == "esc" || input == "escape")
    {
        control.SetKeyboard(Input::Keyboard::Escape);
        return true;
    }
    if (input == "up")
    {
        control.SetKeyboard(Input::Keyboard::Up);
        return true;
    }
    if (input == "down")
    {
        control.SetKeyboard(Input::Keyboard::Down);
        return true;
    }
    if (input == "left")
    {
        control.SetKeyboard(Input::Keyboard::Left);
        return true;
    }
    if (input == "right")
    {
        control.SetKeyboard(Input::Keyboard::Right);
        return true;
    }
...
#endif
{
    ...
    nh.param<std::string>("policy_config", this->config_name, "");
    ...
    {
        std::cout << LOGGER::INFO << "Get param policy_config: " << this->config_name << std::endl;
    }
    ...
    if (FSMManager::GetInstance().IsTypeSupported(this->robot_name))
    {
        ...
        if (fsm_ptr)
        {
            ...
            std::cout << LOGGER::ERROR << "Failed to create FSM for robot: " << this->robot_name << std::endl;
        }
    }
    else
    {
...
void RL_Sim::GetState(RobotState<double> *state)
{
    ...
    // phase = std::fmod(time_now, 0.5) / 0.5;
...
void RL_Sim::RobotControl()
{
    ...
    if (simulation_running)
    {
        ...
        //printf("current_keyboard is: %d\n", this->control.current_keyboard);
...
#elif defined(USE_ROS2)
void RL_Sim::DebugKeyCallback(const std_msgs::msg::String::SharedPtr msg)
{
    ...
    if (input == "quit" || input == "shutdown" || input == "exit")
    {
        std::cout << LOGGER::INFO << "Received debug shutdown command." << std::endl;
        rclcpp::shutdown();
        return;
    }
